...
for fieldid, fieldname in fields.items():
    ...
    #for season in [0]:
        for params in params_sn:
            for simu in ['snsim']:
                for z in zvals:
                    cmd='python multiple_batch.py --fieldname '+fieldname+' --fieldid '+str(fieldid)+' --z '+str(round(z,2))+' --stretch '+str(params[0])+' --color '+str(params[1])+' --dirmeas Light_Curves_'+simu+' --season '+str(season)+' --simulator '+simu+' --dirout Fitted_Light_Curves_'+simu+' --n_per_batch '+str(n_per_batch)
                    print cmd
                    os.system(cmd)
