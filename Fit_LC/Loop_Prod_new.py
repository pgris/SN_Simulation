...
#for z in np.arange(zmin,zmax-zstep,zstep):
    ...
    for val in tab:
        cmd='python fit_lcs.py --z '+str(val['z'])+' --fieldname '+val['fieldname']+' --fieldid '+str(val['fieldid'])+' --season '+str(val['season'])+' --sntype '+val['sntype']+' --stretch '+str(val['x1'])+' --color '+str(val['c'])+' --dirmeas '+val['dirmeas']+' --dirout '+val['dirout']+' --multiproc '+multiproc+' --DayMax '+str(val['DayMax'])
...
if paramfile == '':
    ...
    for par in paramsb:
        ...
        dirout=ddout
        dirmeas=ddout
        if 'Fitted' not in ddout:
            dirout='Fitted_'+ddout
        lo.append(dirout)
        lo.append(dirmeas)
        r.append(lo)
    ...
    for i in range(len(ivals)-1):
        batch(params[ivals[i]:ivals[i+1]],i,multiproc='yes')
