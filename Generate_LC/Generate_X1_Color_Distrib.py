...
def Save_npz(name,x1_mean,sig_m_x1,sig_p_x1,c_mean,sig_m_c,sig_p_c):
    ...
    x1_vals, x1_weights = gauss_asym_distrib(x1_mean,sig_m_x1,sig_p_x1,0.1)
    c_vals, c_weights = gauss_asym_distrib(c_mean,sig_m_c,sig_p_c,0.01)
    
    fichname='Dist_X1_Color_'+name
    np.savez(fichname,x1_vals=x1_vals, x1_weights=x1_weights,c_vals=c_vals, c_weights=c_weights)

def gauss_asym_distrib(mean,sigma_minus,sigma_plus,pas):
