...
#zvals+=[zmax]
print 'hello',zmin,zmax,zvals
for z in zvals:
    zv=z
    if z == 0.:
        zv=0.01
    cmd='python generate_lc.py --z '+str(zv)+' --fieldname '+fieldname+' --fieldid '+str(fieldid)+' --season '+str(season)+' --sntype '+sntype+' --stretch '+str(stretch)+' --color '+str(color)+' --dirmeas '+dirmeas+' --T0min '+str(T0min)+' --T0max '+str(T0max)+' --dirout '+dirout+ ' --T0step '+str(T0step)
    if X1C != '':
        cmd+=' --X1C '+X1C
    script.write(cmd+" \n")
script.write("EOF" + "\n")
