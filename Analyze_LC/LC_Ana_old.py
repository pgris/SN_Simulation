...
class LC_Ana:
    def __init__(self,data_dict={},zmin=0.,zmax=1.2,bin_z=0.01,data_dir='../../Fitted_Light_Curves_snsim',fieldids=[]):
        ...
        for fieldid in fieldids:
            tot_data[fieldid]=self.Load_Data_Multiproc(data_dict,fieldid)
        ...
        #data_ref=self.data.copy()
        
        selection={}

        sela=[]
        
        sela.append((['N_Phase_m5'],operator.ge,1))
        sela.append((['N_Phase_p30'],operator.ge,1))

        sela.append((['N_nights_m20_p_p30'],operator.ge,7))
        sela.append((['Near_peak_gap'],operator.lt,15.))
        thecut=[]
        snrcut=10
        for band in 'grizy':
            thecut.append('N_'+band+'_snrmax_'+str(snrcut))

        sela.append((thecut,operator.ge,2)) 
        sela.append((['status'],operator.eq,'go_fit'))
        sela.append((['fit_status'],operator.eq,'fit_ok'))
        selection['Sience_Book_no_colorcut']=sela
        selac=list(sela)
        selac.append((['salt2.CovColorColor'],operator.le,0.0016))
        #selection['Sience_Book_with_colorcut']=selac


        #self.mysel.append((['salt2.CovColorColor'],operator.le,0.0016))
        #self.myself.append((,operator.ge,1))

        selb=[]
        selb.append((['N_bef_all'],operator.ge,4))
    
        selb.append((['N_aft_all'],operator.ge,10))
        selb.append((['status'],operator.eq,'go_fit'))
        selb.append((['fit_status'],operator.eq,'fit_ok'))
        
        selb.append((['phase_first'],operator.le,-5))
        selb.append((['phase_last'],operator.ge,20))
        
        #self.mysel.append(('salt2.CovColorColor',operator.le,0.0016))
        #selection['Sela_no_color_cut']=selb
        selbc=list(selb)
        selbc.append((['salt2.CovColorColor'],operator.le,0.0016))
        #selection['Sela_with_color_cut']=selbc

        #self.Plot(data_dict,sel_all,0.575)

        #for key, val in selection.items():
            #self.Estimate_Efficiencies(data_dict,val,key,interp=True)
        """
        eff_zero=[]
        for fieldid in [290,744,1427,2412,2786]:
            for x1_color in [(-2.0,0.2),(2.0,-0.2),(0.0,0.0)]:
        #for x1_color in [(-2.0,0.2)]:
                self.Plot_Efficiencies_Multiple(fieldid,data_dict,selection,x1_color,eff_zero)
        #self.Plot_Var(data_dict,'z')

        #print(eff_zero)
        zeros=np.rec.fromrecords(eff_zero,names=['fieldid','season','z','X1','Color'])
        np.save('Toreprocess.npy',zeros)
        print(zeros)
        """

        for keya in tot_data.keys:
            for key,selec in selection.items():
                self.Check_Zero_Efficiencies(tot_data[keya],selec)


    def Check_Zero_Efficiencies(self,tot_data,selec):
        zlim=dict(zip([-2.0,0.0,2.0],[0.75,1.,1.]))
        print(tot_data.dtype)
        r=[]
        sel_data=self.Select_all(tot_data,selec)
        for fieldid in np.unique(tot_data['fieldid']):
            for x1_color in np.unique(tot_data[['X1','Color']]):
                for z in np.unique(tot_data['z']):
                    for season in np.unique(tot_data['season']):
                        idxa=(tot_data['fieldid']==fieldid)&(tot_data['z']==z)&(tot_data[['X1','Color']]==x1_color)&(tot_data['season']==season)
                        idxb=(sel_data['fieldid']==fieldid)&(sel_data['z']==z)&(sel_data[['X1','Color']]==x1_color)&(sel_data['season']==season)
                        #print(fieldid,z,len(sel_data[idxb]),len(tot_data[idxa]))
                        if len(sel_data[idxb])==0 and z < zlim[x1_color[0]]:
                            print('To reprocess',fieldid,season,z,x1_color)
                            r.append((fieldid,season,z,x1_color[0],x1_color[1]))
        if len(r) > 0:
    ...
    def Get_Efficiencies(self,data_dict,selec):
        ...
        for key,vals in self.data.items():
            #print 'hello',key,len(vals),vals.dtype
            #outname='NSN_'+data_dict[key].simul_name+'_'+selname
            if vals is not None:
                #print(vals.dtype)
                valscp=vals.copy()
                for ic,sel in enumerate(selec):
                    valscp=self.Select_Cut(valscp,sel[0],sel[1],sel[2])
                    #print('after selection',sel[0],valscp['N_Phase_p30'],valscp['max_rf_phase'],len(valscp),len(vals),data_dict[key].fieldname,data_dict[key].fieldid,data_dict[key].z,data_dict[key].season)
            
                effi=float(len(valscp))/float(len(vals))
                err_effi=np.sqrt(1.-effi)*effi/np.sqrt(float(len(vals)))
                r.append((data_dict[key].fieldname,data_dict[key].fieldid,data_dict[key].z,data_dict[key].season,data_dict[key].X1,data_dict[key].Color,effi,err_effi,vals['duration'][0]))

        #plt.show()
        print 'helli',len(r)
        return np.rec.fromrecords(r,names=['fieldname','fieldid','z','season','X1','Color','effi','err_effi','duration'])
    ...
    def Load_Indiv(self,key,val,j,output_q=None):
    ...
    def Load_Data_Multiproc(self,data_dict_tot={},fieldid=0):

        n_per_batch=1

        data_dict=dict((key,value) for key, value in data_dict_tot.iteritems() if str(fieldid) in key)

        
        inter=range(0,len(data_dict.keys()),n_per_batch)
        inter=np.append(inter,len(data_dict.keys()))
        ...
        for jo in range(len(inter)-1):
            ...
            #print('radec',j,radec[j])
                thekey=data_dict.keys()[j]
                p=multiprocessing.Process(name='Subprocess-'+str(j),target=self.Load_Indiv,args=(thekey,data_dict[thekey],j,result_queue))
                p.start()
